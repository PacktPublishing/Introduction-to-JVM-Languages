...
class AnimatedText {
	...
	val timer = object : AnimationTimer() {
		override fun handle(now: Long) {
			...
			} else if (animatedText.x > animationPane.width - animatedText.layoutBounds.width) {
				animatedText.x = animationPane.width - animatedText.layoutBounds.width
				directionX = -directionX
			}
	...
	fun getPane(textField: TextField): Pane {
		animatedText.x = 0.0
		animatedText.y = 0.0
		animatedText.font = Font.font("Verdana", FontWeight.BOLD, 30.0)
